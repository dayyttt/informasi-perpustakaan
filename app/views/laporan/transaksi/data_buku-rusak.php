...
<div class="mb-5">
	<div class="mb-3">
		...
		<b><?= sum("buku_rusak", "jumlah_buku") ?> buah</b>
	</div>
	<div class="mb-3">
		<p>Total Ganti Rugi Dibayarkan</p>
		<b><?= rp(Capsule::table("buku_rusak")->selectRaw("SUM(jumlah_buku * harga) AS total_kerugian")->where("status", 1)->first()->total_kerugian) ?></b>
	</div>
</div>
<table>
	<thead>
	...
	<tbody>
		<?php if(count(all("buku_rusak")) == 0) : ?>
			<tr>
				<td colspan="100%" style="text-align: center">Tidak ada data</td>
			</tr>
		<?php endif ?>
		 <?php foreach (all("buku_rusak") as $key => $buku_hilang) : ?>
			<tr>
				<td><?= $key + 1 ?></td>
