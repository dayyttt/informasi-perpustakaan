...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (validation(editGuruRules(), $_POST)) {
        if ($_FILES["gambar"]["name"] != "") {
            ...
            if (in_array($ext, $validExtension) === true) {
                if ($size < $maxSize) {
                    ...
                    return header("Location: /guru/edit?id=$id");
                }
            } else {
        ...
        if(get("guru", $id)->username != $columns["username"]) {
            if(!preg_match("/^\S*$/u", $columns["username"])) {
                setAlertSession("Data gagal ditambahkan! Username tidak boleh mengandung spasi!", "danger");
                return header("Location: /guru/edit?id=$id");
            } else if(amountWhere("guru", "username", $columns["username"]) == 1 || amountWhere("pengguna", "username", $columns["username"]) == 1) {
